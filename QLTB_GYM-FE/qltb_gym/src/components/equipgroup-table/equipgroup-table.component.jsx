...
import { deleteEquipGroup } from '../../api/EquipmentGroupApi';
...
const handleDel = async (id, delEquipGroup) => {
  try {
    const response = await deleteEquipGroup(id)
...
const EquipGroupTable = ({ equipGroup,setEquipGroup, history, match, delEquipGroup, user }) => {
  const [show, setShow] = useState(false);
  const [dataTemp, setDataTemp] = useState([]);
console.log("EquipGroupTable")
  const dataArray = equipGroup.map(
    (x, index) => ({
      order: `${index + 1}`,
      id: x.id,
      name: x.equipGroupName,
      // des: 'không có',
      // passed: true,

    })
  );
  const DeleteButton = ({ rowData, delEquipGroup }) => {
    return (
      <div className='edit-cell-button'>
        <RiDeleteBin2Fill
          alt='Delete Row'
          ...
          onClick={() => {
            setShow(true);
            setDataTemp([rowData.id, delEquipGroup])
  ...
  const tablePropsInit = {
    columns: [
      ...
      { key: 'name', title: 'TÊN LOẠI THIẾT BỊ', dataType: DataType.String, style: { width: 300 } },
  ...
  const handleSetequipGroup = (id)=>{
    const newSup = equipGroup.filter((equip)=>equip.id !== id)
    setEquipGroup(newSup)
  }
    ...
    <div>
      <Modal show={show} onHide={handleClose}>
        <Modal.Header closeButton>
          <Modal.Title>Xóa loại thiết bị?</Modal.Title>
        </Modal.Header>
        <Modal.Body>Xác nhận xóa loại thiết bị!</Modal.Body>
        <Modal.Footer>
          <Button variant="secondary" onClick={() => {
            handleDel(dataTemp[0], dataTemp[1], dataTemp[2]);
            setShow(false);
            handleSetequipGroup(dataTemp[0])
          }}>
            OK
          </Button>
          ...
          cellText: {
            content: (props) => {
              ...
              if (props.column.key === 'deleteColumn') {
                return <DeleteButton {...props} delEquipGroup={delEquipGroup} user={user} />
              }
            }
          },
          noDataRow: {
...
const mapDispatchToProps = dispatch => ({
   delEquipGroup: id => dispatch(deleteEquipmentGroup(id))
})


export default withRouter(connect(mapStateToProps, mapDispatchToProps)(EquipGroupTable));